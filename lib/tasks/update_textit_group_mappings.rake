...
task :update_textit_group_mappings => :environment do
  logger = Logger.new("#{Rails.root}/log/update_campaign_trails_v2/update_textit_group_mappings.log")
  # first read the file from disk where the dump is stored
  if Rails.env.staging? || Rails.env.production?
    dump_file = File.read("#{Rails.root}/log/update_campaign_trails_v2/users_textit_group_mappings.json")
  else
